...
/**
 * 获取所有账户的登录信息
 * @returns {Array} 账户列表，按time_update降序排序
 */
  ...
  try {
    ...
    snap.forEach(node => {
      ...
      auths.push({
        ...
        token: data.token,
        cookies: cookiesStr,
        cookiesObj: data.cookies,
        providerIdM: String(data.provider_id_m || ''),
        providerName: data.provider_name || accountName,
    ...
    // 按time_update降序排序（最新的在前）
    auths.sort((a, b) => {
      const timeA = a.timeUpdate || '';
      const timeB = b.timeUpdate || '';
...
/**
 * 获取最新的账户登录信息（兼容旧接口）
 * @returns {Object|null} { token, cookies, providerIdM, accountName }
 */
 ...
 * @returns {Object|null}
 */
export async function getAuthByAccount(accountName) {
  try {
    const snap = await get(child(dbRef, `zhanghu/${accountName}`));
...
/**
 * 保存账户登录信息
 * @param {string} accountName - 账户名称（作为节点key）
 * @param {Object} authData - 登录信息
 */
export async function saveAuth(accountName, authData) {
  try {
    await set(ref(db, `zhanghu/${accountName}`), authData);
    console.log(`账户 ${accountName} 登录信息已保存`);
    return true;
  } catch (e) {
    console.error('保存账户信息失败：', e);
    throw e;
  }
}
 ...
 */
export async function updateAuth(accountName, updates) {
  try {
    await update(ref(db, `zhanghu/${accountName}`), updates);
    return true;
  } catch (e) {
 ...
 * @param {string} accountName - 账户名称
 */
export async function deleteAuth(accountName) {
  try {
    await remove(ref(db, `zhanghu/${accountName}`));
    return true;
  } catch (e) {
    console.error('删除账户信息失败：', e);
    throw e;
  }
}

/**
 * 标记账户凭证失效（更新时间戳使其排到最后）
 * @param {string} accountName - 账户名称
 */
export async function markAuthInvalid(accountName) {
  try {
    await update(ref(db, `zhanghu/${accountName}`), {
      time_update: '1970/01/01 00:00:00' // 设置为很早的时间，使其排到最后
    });
    return true;
  } catch (e) {
    console.error('标记账户失效失败：', e);
    return false;
  }
}
