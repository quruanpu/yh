...
const CjYewu = {
    ...
    collectFormData() {
        ...
        if (this.currentType === 13) {
            ...
            data.price = Number(this.val('price'));
        }
    ...
    validate(data) {
        ...
        if (!data.minPay || data.minPay <= 0) return '请输入有效的使用条件';

        if (data.type === 13 && (!data.discount || data.discount <= 0)) return '请输入有效的折扣';
        if (data.type === 11 && (!data.price || data.price <= 0)) return '请输入有效的优惠金额';
    ...
    async handleSubmit() {
        ...
        try {
            ...
            this.notify('创建失败：' + (err.message || '未知错误。'), 'error');
        } finally {
            btn.disabled = false;
            btn.textContent = '创建';
        }
