...
const AIToolsModule = {
    ...
    tools: [
        ...
        },
        {
            ...
            function: {
                name: 'search_product',
                description: '根据商品编码、药品名称或国药准字查询商品信息。当用户上传药品图片时，先识别图片中的商品编码/药品名称/国药准字，然后调用此工具查询。',
                parameters: {
                    type: 'object',
                    properties: {
                        keyword: {
    ...
    async executeTool(toolName, args, sessionId) {
        ...
        try {
            switch (toolName) {
                ...
                case 'search_product':
                    return await this.searchProduct(args.keyword);
    ...
    async searchProduct(keyword) {
        ...
        if (!window.ChaxunAPIModule) {
            return { error: '商品查询模块未加载' };
        }

        try {
            const result = await ChaxunAPIModule.searchProducts(keyword.trim(), [], 0);

            if (!result.success) {
                if (result.error === 'NO_LOGIN') {
                    return { error: '请先登录SCM账户' };
                }
                return { error: result.error || '查询失败' };
            }
            ...
            return {
                ...
                products: products,
                render_cards: true,
                message: `已查询到${products.length}个商品`
            };
        } catch (error) {
