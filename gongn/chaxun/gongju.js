...
const GongjuApi = {
    ...
    async getCredentials() {
        ...
        if (!result.ok || !result.credentials) return null;
    ...
    async requirePmsCredentials() {
        if (!window.LoginModule) {
            console.warn('登录模块未加载。');
            return null;
        }
        const result = await window.LoginModule.requireCredentials('pms', { silent: true });
        if (!result.ok || !result.credentials) return null;
        return result.credentials;
    },

    normalizePmsAuth(credentials = {}) {
        const token = credentials.token
            || credentials.accessToken
            || credentials.access_token
            || credentials.pmsToken
            || credentials.pms_token
            || '';
        return token ? { ...credentials, token } : { ...credentials };
    },

    isPmsAuthError(message = '') {
        return /登录|登陆|未登录|重新登录|token|auth\.token|凭证|过期|失效|unauthorized|expired|invalid token|login required/i
            .test(String(message || ''));
    },

    async queryPmsContactor(drugCode) {
        try {
            const credentials = await this.requirePmsCredentials();
            const auth = this.normalizePmsAuth(credentials || {});
            if (!auth) {
                return { success: false, error: 'NO_PMS_LOGIN', message: '请先登录PMS账户' };
            }
            if (!auth.token) {
                return { success: false, error: 'PMS_AUTH_INCOMPLETE', message: 'PMS凭证缺少查询所需token' };
            }
            ...
            if (result.code !== 0) {
                ...
                return {
                    success: false,
                    error: this.isPmsAuthError(msg) ? 'NO_PMS_LOGIN' : 'PMS_QUERY_FAILED',
                    message: msg || '查询失败'
                };
            }
            ...
            if (products.length === 0) {
                return { success: false, error: 'NO_MATCH', message: '未找到商品' };
            }
    ...
    async searchProducts(keyword = '', wholesaleTypes = [], status = -1, options = {}) {
        ...
        try {
            ...
            if (options && typeof options === 'object' && Object.prototype.hasOwnProperty.call(options, 'includeImages')) {
                query.includeImages = !!options.includeImages;
            }

            const requestBody = { auth: auth, query: query };
            ...
            if (result.code !== 0) {
                const msg = result.message || '';
                return { success: false, error: 'NO_LOGIN', message: msg || '查询失败' };
            }
