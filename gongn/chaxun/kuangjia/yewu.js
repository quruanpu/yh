...
const KuangjiaYewu = {
    ...
    listenLoginSuccess() {
        ...
        document.addEventListener('loginSuccess', (e) => {
            if (e.detail.system === 'PMS' && this._pendingPmsQuery) {
                setTimeout(() => {
                    if (this._pendingPmsQuery) {
                        // 检查DOM元素是否还存在
                        if (document.contains(this._pendingPmsQuery.eyeIcon)) {
                            this.queryContactor(this._pendingPmsQuery.eyeIcon);
                        }
                        this._pendingPmsQuery = null;
                    }
                }, 500);
            }
        });
        ...
        // 监听登录窗口关闭（用户取消登录）
        document.addEventListener('loginClosed', () => {
            if (this._pendingPmsQuery) {
                const { valueSpan, eyeIcon } = this._pendingPmsQuery;
                if (valueSpan && document.contains(valueSpan)) {
                    valueSpan.textContent = '已取消';
                    valueSpan.style.color = '#999';
                }
                if (eyeIcon && document.contains(eyeIcon)) {
    ...
    async queryContactor(eyeIcon) {
        ...
        if (result.success) {
            ...
            if (result.error === 'NO_PMS_LOGIN') {
                valueSpan.textContent = '登录中...';
                valueSpan.style.color = '#faad14';
                // 保存查询上下文，等待登录成功事件
                this._pendingPmsQuery = { eyeIcon, drugCode, valueSpan };
                // 直接弹出PMS登录窗口
                if (window.LoginModule) {
                    LoginModule.open('pms');
                }
            } else {
