...
const ScmLoginModule = {
    ...
    async recognizeCaptcha(captchaBase64) {
        if (!this.config.ocrUrl) {
            console.warn('OCR接口未配置');
            return null;
        }
        try {
            ...
            if (error.name === 'AbortError') {
                console.warn('OCR识别超时（10秒），请手动输入验证码');
            } else {
